...
STAGED_APP="$STAGE_ROOT/Stick S3 虚拟设备.app"
...
cp "$PROJECT_DIR/.build/release/StickS3Simulator" "$STAGED_APP/Contents/MacOS/StickS3Simulator"
...
# 公共源码的本机构建默认使用可复现的 ad-hoc 签名；发布者可显式传入身份。
...
if [[ -e "$APP_DIR" ]]; then
    [[ "$APP_DIR" == "$PROJECT_DIR/build/Stick S3 虚拟设备.app" ]] || exit 70
    /bin/rm -rf -- "$APP_DIR"
fi
