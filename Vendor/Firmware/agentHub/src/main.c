...
static bool s_provider_selector_active = true;
...
static lv_obj_t *s_provider_selector_cards[PROVIDER_COUNT];
static lv_obj_t *s_provider_selector_hints[PROVIDER_COUNT];
...
static const agent_provider_config_t s_provider_configs[] = {
    ...
    },
    {
        ...
        .icon = NULL,
        .accent_color = LV_COLOR_MAKE(0x7c, 0x6c, 0xff),
    },
};
...
static void update_provider_selector(void);
...
static void create_provider_selector_ui(lv_obj_t *screen)
{
    lv_obj_set_style_bg_color(screen, lv_color_hex(0x030711), 0);
    lv_obj_set_style_bg_opa(screen, LV_OPA_COVER, 0);
    lv_obj_set_style_pad_all(screen, 0, 0);

    lv_obj_t *title = make_label(screen, "CHOOSE AGENT",
                                 &lv_font_montserrat_12,
                                 lv_color_hex(0xf7f9ff), 125,
                                 LV_TEXT_ALIGN_CENTER);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 8);
    lv_obj_t *subtitle = make_label(screen, "VIBESTICK AGENT HUB",
                                    &lv_font_montserrat_10,
                                    lv_color_hex(0x6782ad), 125,
                                    LV_TEXT_ALIGN_CENTER);
    lv_obj_align(subtitle, LV_ALIGN_TOP_MID, 0, 21);

    for (int i = 0; i < PROVIDER_COUNT; ++i) {
        const agent_provider_config_t *provider = provider_config((agent_provider_t)i);
        lv_obj_t *card = make_plain_obj(screen, 125, 53,
                                        lv_color_hex(0x08101f),
                                        LV_OPA_COVER, 12);
        lv_obj_set_pos(card, 5, 36 + i * 58);
        lv_obj_set_style_border_width(card, 1, 0);
        lv_obj_set_style_border_color(card, lv_color_hex(0x28436f), 0);
        s_provider_selector_cards[i] = card;
        const char *glyph = i == PROVIDER_CODEX
                                ? "[>_]"
                                : (i == PROVIDER_CLAUDE_CODE ? "(*)" : "[K.]");
        lv_obj_t *icon = make_label(screen, glyph, &lv_font_montserrat_16,
                                    lv_color_hex(0x7898cf), 40,
                                    LV_TEXT_ALIGN_CENTER);
        lv_obj_set_pos(icon, 8, 52 + i * 58);

        const lv_font_t *name_font = i == PROVIDER_CLAUDE_CODE
                                         ? &lv_font_montserrat_10
                                         : &lv_font_montserrat_14;
        lv_obj_t *name = make_label(screen, provider->display_name, name_font,
                                    lv_color_hex(0xf7f9ff), 64,
                                    LV_TEXT_ALIGN_LEFT);
        lv_obj_set_pos(name, 51,
                       51 + i * 58 + (i == PROVIDER_CLAUDE_CODE ? 2 : 0));
        lv_obj_t *arrow = make_label(screen, ">", &lv_font_montserrat_20,
                                     lv_color_hex(0x6f9be8), 13,
                                     LV_TEXT_ALIGN_CENTER);
        lv_obj_set_pos(arrow, 114, 48 + i * 58);
        s_provider_selector_hints[i] = arrow;
    }

    lv_obj_t *hint = make_label(screen, "SIDE: SELECT   FRONT: ENTER",
                                &lv_font_montserrat_10,
                                lv_color_hex(0x76849b), 130,
                                LV_TEXT_ALIGN_CENTER);
    lv_obj_align(hint, LV_ALIGN_BOTTOM_MID, 0, -5);
    update_provider_selector();
}

static void update_provider_selector(void)
{
    for (int i = 0; i < PROVIDER_COUNT; ++i) {
        if (!s_provider_selector_cards[i]) {
            continue;
        }
        bool selected = i == (int)s_current_provider;
        const agent_provider_config_t *provider = provider_config((agent_provider_t)i);
        lv_obj_set_style_border_width(
            s_provider_selector_cards[i], selected ? 2 : 1, 0);
        lv_obj_set_style_border_color(
            s_provider_selector_cards[i],
            selected ? provider->accent_color : lv_color_hex(0x28436f), 0);
        lv_obj_set_style_bg_color(
            s_provider_selector_cards[i],
            selected ? lv_color_hex(0x101a2c) : lv_color_hex(0x08101f), 0);
        if (s_provider_selector_hints[i]) {
            lv_obj_set_style_text_color(
                s_provider_selector_hints[i],
                selected ? provider->accent_color : lv_color_hex(0x51688f), 0);
        }
    }
}

static void load_provider_dashboard(void)
{
    lvgl_lock();
    lv_obj_t *old_screen = lv_display_get_screen_active(s_display);
    lv_obj_t *new_screen = lv_obj_create(NULL);
    s_provider_selector_active = false;
    memset(s_provider_selector_cards, 0, sizeof(s_provider_selector_cards));
    memset(s_provider_selector_hints, 0, sizeof(s_provider_selector_hints));
...
static void post_provider_selection(void)
{
    const agent_provider_config_t *provider = current_provider_config();
    char body[128];
    snprintf(body, sizeof(body),
             "{\"event\":\"provider_select\",\"provider_id\":\"%s\",\"source\":\"sticks3\"}",
             provider->key);
    char response[512] = {0};
...
static void orientation_task(void *arg)
{
    ...
    while (true) {
        ...
        if (s_provider_selector_active || !s_orientation_enabled ||
...
static void app_task(void *arg)
{
    ...
    while (true) {
        ...
        if (!s_provider_selector_active && s_wifi_connected &&
        ...
        if (s_provider_selector_active) {
            if (event.type == VIBE_STICK_EVENT_SIDE_SHORT ||
                event.type == VIBE_STICK_EVENT_SIDE_TRIPLE) {
                s_current_provider = (agent_provider_t)(
                    ((int)s_current_provider + 1) % PROVIDER_COUNT);
                render_state();
            } else if (event.type == VIBE_STICK_EVENT_SHORT_PRESS) {
                ESP_LOGI(TAG, "provider enter=%s", current_provider_config()->key);
                load_provider_dashboard();
                post_provider_selection();
                poll_state();
            }
            continue;
        }
        ...
        case VIBE_STICK_EVENT_SIDE_TRIPLE:
            s_current_provider = (agent_provider_t)(
                ((int)s_current_provider + 1) % PROVIDER_COUNT);
            ESP_LOGI(TAG, "provider switch=%s", current_provider_config()->key);
            render_state();
            post_provider_selection();
            break;
