...
static CodexHostContext *s_host_context;

#define VIRTUAL_DEVICE_HOST 1
#ifndef CODEX_FIRMWARE_MAIN
#define CODEX_FIRMWARE_MAIN "../../Vendor/Firmware/codex/src/main.c"
#endif
#include CODEX_FIRMWARE_MAIN
...
{
    CodexHostContext *context = lv_display_get_user_data(display);
    if (!context) return;
...
{
    CodexHostContext *context = calloc(1, sizeof(*context));
    ...
    if (!context->framebuffer || !context->draw_buffer) {
        codex_firmware_destroy(context); return NULL;
    }
    s_host_context = context;
    context->last_person_animation = -1;
    context->frame_width = LCD_V_RES;
    context->frame_height = LCD_H_RES;
    if (!lv_is_initialized()) lv_init();
    s_display = lv_display_create(LCD_V_RES, LCD_H_RES);
    ...
    s_landscape_active = true;
    s_landscape_reverse = false;
    s_wifi_connected = true;
    create_landscape_ui(lv_display_get_screen_active(s_display));
    render_state();
...
void codex_firmware_destroy(void *opaque)
{
    CodexHostContext *context = opaque;
    if (!context) return;
    ...
    bool today_used_percent_valid, bool today_tokens_valid)
{
    ...
    provider_display_state_t *provider = &s_provider_states[PROVIDER_CODEX];
...
void codex_firmware_update(void *opaque, uint32_t now_ms)
{
    CodexHostContext *context = opaque;
    if (!context) return;
...
void codex_firmware_set_recording(void *opaque, bool recording)
{
    CodexHostContext *context = opaque;
    if (!context) return;
    s_host_context = context; lv_display_set_default(s_display);
...
void codex_firmware_set_orientation(void *opaque, bool landscape, bool landscape_reverse)
{
    CodexHostContext *context = opaque;
...
CodexFirmwareSnapshot codex_firmware_snapshot(void *opaque)
{
    CodexFirmwareSnapshot result = {0};
...
const uint16_t *codex_firmware_framebuffer(void *opaque) {
    CodexHostContext *context = opaque; return context ? context->framebuffer : NULL;
}
uint32_t codex_firmware_frame_serial(void *opaque) {
    CodexHostContext *context = opaque; return context ? context->frame_serial : 0;
}
int32_t codex_firmware_frame_width(void *opaque) {
    CodexHostContext *context = opaque; return context ? context->frame_width : 0;
}
int32_t codex_firmware_frame_height(void *opaque) {
    CodexHostContext *context = opaque; return context ? context->frame_height : 0;
}
